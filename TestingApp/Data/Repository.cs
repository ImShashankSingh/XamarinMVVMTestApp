...
namespace TestingApp.Data
{
    ...
    {
        const string DatabaseName = "DaVinciLock.db5";
        ...
        static Repository()
        {
            DBPath = System.IO.Path.Combine(System.Environment.GetFolderPath(System.Environment.SpecialFolder.Personal), DatabaseName);
            InitializeDatabase();
        }
        ...
        static void ClearAllTablesData()
        {
            lock (DBConnection)
            {
                DBConnection.DeleteAll<Employee>();
            }
        }
        ...
        public static void InsertOrReplace<T>(T item) where T : IEntity
        {
            lock (DBConnection)
            {
                item.UpdatedLocallyOn = DateTime.UtcNow;
        ...
        public static void SaveOrUpdate<T>(List<T> items) where T : IEntity, new()
        {
            ...
            lock (DBConnection)
            {
                items.ForEach(x => x.UpdatedLocallyOn = DateTime.UtcNow);
        ...
        public static List<T> Find<T>(Func<T, bool> query) where T : IEntity, new()
        {
            ...
            {
                if (query == null)
                    return AsQueryable<T>().OrderByDescending(x => x.UpdatedLocallyOn).ToList();

                return AsQueryable<T>().Where(query).OrderByDescending(x => x.UpdatedLocallyOn).ToList();
            }
            //return Find<T>(query);
        }
        ...
        public static List<T> Find<T>(Func<T, bool> query, int page, int limit) where T : IEntity, new()
        {
            ...
            {
                if (query == null)
                    return AsQueryable<T>().OrderByDescending(x => x.UpdatedLocallyOn)
                        .Skip(page).Take(limit).ToList();
                return AsQueryable<T>().Where(query).OrderByDescending(x => x.UpdatedLocallyOn)
                    .Skip(page).Take(limit).ToList();
            }
        }
